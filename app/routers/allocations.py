...
async def allocation_decision_form(request: Request, allocation_id: int, db: Session = Depends(get_db)):
    ...
    return render(
        ...
        "allocations_decision.html",
        {
            ...
            "page_subtitle": f"{allocation.asset.asset_code} → {allocation.employee.full_name}",
            "page_actions": [{"href": "/allocations", "label": "Back", "variant": "secondary", "icon": "◀️"}],
...
async def sign_allocation_form(request: Request, allocation_id: int, db: Session = Depends(get_db)):
    ...
    return render(
        ...
        "allocations_sign.html",
        {
            ...
            "page_subtitle": f"{allocation.asset.asset_code} — Please sign with OTP to complete the allocation",
            "page_actions": [{"href": "/allocations", "label": "Back", "variant": "secondary", "icon": "◀️"}],
            "active_path": "/allocations",
        },
        current_user,
